...
public class PlasmaCondenserRecipes {
    ...
    public static void init() {
        for(Material material : materials) {
            ...
            if (prop.get(FluidStorageKeys.LIQUID) == null || prop.get(FluidStorageKeys.PLASMA) == null) {continue;}

            boolean isExotic = false;
            for(Material exoticMaterial : new Material[] {Flerovium, Neutronium, Draconium, AwakenedDraconium, Infinity, Dragonblood, Rhugnor, Hypogen, ChromaticGlass, Shirabon, Periodicium}) {
                if(material == exoticMaterial){isExotic = true;break;}
            }
            ...
            PLASMA_CONDENSER_RECIPES.recipeBuilder()
                    .fluidInputs(BlackStarMatter.getFluid(isExotic ? 1000 : 10),
                            material.getPlasma(isSolid ? 144*8 : 1000))
                    .fluidOutputs(material.getFluid(isSolid ? 144*8 : 1000))
            ...
            PLASMA_CONDENSER_RECIPES.recipeBuilder()
                    .fluidInputs(CondensedStarMatter.getFluid(isExotic ? 1000 : 10),
                            material.getFluid(isSolid ? 144*8 : 1000))
                    .fluidOutputs(material.getPlasma(isSolid ? 144*8 : 1000))
                    .duration(isExotic ? 32*20 : 32).EUt(GTValues.VA[isExotic ? GTValues.MAX : GTValues.UEV]).buildAndRegister();
        }
    }
}
